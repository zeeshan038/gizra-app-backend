...
exports.finalizeFtCursor = exports.routeFtCursor = exports.extractCursorValue = exports.upperCommand = exports.argToString = void 0;
 ...
 */
function upperCommand(parser) {
    return parser.commandIdentifier.command.toUpperCase();
}
exports.upperCommand = upperCommand;
/**
 ...
 */
function extractCursorValue(reply) {
    if (reply == null)
        return undefined;
    ...
    if (typeof reply === 'object' && 'cursor' in reply) {
        return asCursorValue(reply.cursor);
    }
    return undefined;
}
exports.extractCursorValue = extractCursorValue;
function asCursorValue(value) {
    if (typeof value === 'number' || typeof value === 'string')
        return value;
    return undefined;
}
function isExhausted(cursor) {
 ...
 */
function maxIdleFromAggregateArgs(redisArgs) {
 ...
 * out or guess.
 */
const routeFtCursor = async (slots, parser) => {
    ...
    if (binding) {
        ...
        if (client)
            return [{ client, parser: withCursorArg(parser, binding.cursorId) }];
        throw new Error(`FT.CURSOR: the node serving cursor ${token} on index "${argToString(redisArgs[2])}" ` +
            `has left the cluster.`);
    }
    throw new Error(`FT.CURSOR: unknown cursor ${token} on index "${argToString(redisArgs[2])}". ` +
...
exports.routeFtCursor = routeFtCursor;
/** Copy of the FT.CURSOR parser with the cursor argument (index 3) replaced. */
function withCursorArg(parser, cursorId) {
    const sub = new parser_1.BasicCommandParser();
    const { redisArgs } = parser;
    for (let i = 0; i < redisArgs.length; i++) {
        sub.push(i === 3 ? cursorId : redisArgs[i]);
    }
    return sub;
}
/**
 ...
 */
function finalizeFtCursor(slots, parser, plan, reply) {
    const cmd = upperCommand(parser);
    if (cmd !== 'FT.AGGREGATE' && cmd !== 'FT.CURSOR')
        return reply;
    if (plan.length !== 1)
        return reply;
    const { redisArgs } = parser;
    if (cmd === 'FT.AGGREGATE') {
        ...
        if (cursor === undefined || isExhausted(cursor))
            return reply;
    ...
    if (sub === 'READ') {
        const binding = slots.lookupCursor(token);
        if (!binding)
            return reply; // never bound here — nothing to maintain
...
exports.finalizeFtCursor = finalizeFtCursor;
/**
 ...
 */
function withReplyCursor(reply, token, original) {
    const cursor = typeof original === 'number' ? Number(token) : token;
    if (reply instanceof Map) {
        const copy = new Map(reply);
...
//# sourceMappingURL=ft-cursor.js.map