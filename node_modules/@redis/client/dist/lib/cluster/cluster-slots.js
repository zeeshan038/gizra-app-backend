...
exports.splitInFlightChainTail = exports.groupCommandsByDestination = exports.RESUBSCRIBE_LISTENERS_EVENT = void 0;
...
function groupCommandsByDestination(commands, slots, fallback) {
...
exports.groupCommandsByDestination = groupCommandsByDestination;
/**
 ...
 * chain intact.
 */
function splitInFlightChainTail(commands, chainInExecution) {
...
exports.splitInFlightChainTail = splitInFlightChainTail;
                ...
                if (sourceStillHasSlots) {
                    ...
                    if (remainingCommands.length > 0) {
                        ...
                        if (inFlightChainTail.length > 0) {
                            sourceNode.client._getQueue().rejectCommands(inFlightChainTail, new errors_1.DisconnectsClientError());
                            (0, enterprise_maintenance_manager_1.dbgMaintenance)(`[CSlots]: Rejected ${inFlightChainTail.length} commands from a chain already partially sent to the server, can't be safely relocated`);
                        }
                        if (relocatable.length > 0) {
                            ...
                            if (unrouted.length > 0) {
                                sourceNode.client._getQueue().rejectCommands(unrouted, new errors_1.DisconnectsClientError());
                                (0, enterprise_maintenance_manager_1.dbgMaintenance)(`[CSlots]: Rejected ${unrouted.length} commands with no resolvable slot owner and no fallback destination`);
                            }
                            for (const [destNode, commands] of commandsByDestination) {
                                if (destNode.client) {
                                    ...
                                    (0, enterprise_maintenance_manager_1.dbgMaintenance)(`[CSlots]: Moved ${commands.length} remaining commands to ${destNode.address}`);
                                }
                                else {
                                    sourceNode.client._getQueue().rejectCommands(commands, new errors_1.DisconnectsClientError());
                                    (0, enterprise_maintenance_manager_1.dbgMaintenance)(`[CSlots]: Rejected ${commands.length} remaining commands because ${destNode.address} has no client`);
                                }
                            }
                        }
                    }
